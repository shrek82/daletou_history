# AI选号策略优化 Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 重写 `examples/ai_pick.rs`，增加6个新分析维度（遗漏值、加权频率、区间分布、尾数、和值、连号），用加权打分驱动生成6组推荐方案，并表格化输出。

**Architecture:** 在现有单文件 example 上扩展，保持 `Stats` 分析结果 → `generate_picks()` 生成方案 → `score_pick()` 评分 → `print_analysis()` 表格输出的数据流。

**Tech Stack:** Rust (standard library + HashMap)
...
| 文件 | 操作 | 职责 |
|------|------|------|
| `examples/ai_pick.rs` | 修改 | 唯一修改文件，包含所有分析、选号、评分、输出逻辑 |
| `src/types.rs` | 不修改 | DrawRecord 结构不变 |
| `src/client.rs` | 不修改 | 缓存机制已满足需求 |

---

## Chunk 1: 分析维度扩展

### Task 1: 扩展 Stats 结构体和常量

**Files:**
- Modify: `examples/ai_pick.rs:53-77`（Stats 结构体）

- [ ] **Step 1: 替换现有 Stats 结构体**
...
struct Stats {
    ...
    /// 红球加权得分（近10期×5 + 近20期×3 + 近50期×2 + 全量×1）
...
```bash
cargo build --example ai_pick 2>&1
```
Expected: 编译失败（Stats 结构体变了但 analyze() 还没更新），这是预期的

- [ ] **Step 3: Commit**

```bash
git add examples/ai_pick.rs
git commit -m "refactor(ai_pick): 扩展 Stats 结构体，新增分析维度字段"
```

---

### Task 2: 重写 analyze() 函数

**Files:**
- Modify: `examples/ai_pick.rs:79-161`（analyze 函数）

- [ ] **Step 1: 替换 analyze() 函数**
...
fn analyze(records: &[DrawRecord]) -> Stats {
    ...
    // === 2. 多时间窗口频率 ===
    let windows = [10, 20, 50];
    let mut red_window_freq: Vec<HashMap<u8, u32>> = windows.iter()
        .map(|&w| count_window_freq(records, w, |r| &r.balls.red))
        .collect();
    let mut blue_window_freq: Vec<HashMap<u8, u32>> = windows.iter()
...
cargo build --example ai_pick 2>&1
```
Expected: 编译失败（print_analysis 还引用旧字段），预期中

- [ ] **Step 4: Commit**

```bash
git add examples/ai_pick.rs
git commit -m "feat(ai_pick): 重写 analyze() 函数，实现6个新分析维度"
```
...
**Files:**
- Modify: `examples/ai_pick.rs:223-310`（generate_picks 函数）

- [ ] **Step 1: 替换 generate_picks() 函数**
...
fn generate_picks(stats: &Stats) -> Vec<Pick> {
    ...
    // 方案2: 冷热混合（3热 + 2冷）
    let red2 = pick_hot_cold_mix(&stats.red_weighted, &stats.red_omission, 3, 2, RED_MAX);
...
fn pick_hot_cold_mix(weighted: &HashMap<u8, f64>, omission: &[u32],
                      hot_count: usize, cold_count: usize, max: u8) -> Vec<u8> {
...
fn pick_by_tail_constraint(weighted: &HashMap<u8, f64>) -> Vec<u8> {
    let candidates = pick_by_weighted_top(weighted, 15);
    greedy_pick_with_tail_constraint(&candidates, weighted, RED_COUNT)
}

/// 贪心选号，满足同尾约束
fn greedy_pick_with_tail_constraint(candidates: &[u8], weighted: &HashMap<u8, f64>, count: usize) -> Vec<u8> {
    let mut result = Vec::new();
...
fn pick_by_consecutive(weighted: &HashMap<u8, f64>) -> Vec<u8> {
    ...
    let mut result = match best_pair {
        Some((a, b)) => vec![a, b],
        None => return pick_by_weighted_top(weighted, RED_COUNT), // 无连号则用方案1
    };
...
fn pick_blue_hot(_weighted: &HashMap<u8, f64>, recent_hot: &[u8]) -> Vec<u8> {
    ...
    if result.len() < BLUE_COUNT {
        for &n in pick_by_weighted_top(weighted, BLUE_COUNT + 2).iter() {
            if !result.contains(&n) {
                result.push(n);
                if result.len() == BLUE_COUNT { break; }
            }
        }
    }
    result.sort();
...
```bash
git add examples/ai_pick.rs
git commit -m "feat(ai_pick): 实现6组推荐策略（纯热号/冷热混合/区间均衡/和值约束/同尾约束/连号策略）"
```
...
**Files:**
- Modify: `examples/ai_pick.rs`（在 generate_picks 之后添加）

- [ ] **Step 1: 新增 score_pick() 函数**
...
```bash
git add examples/ai_pick.rs
git commit -m "feat(ai_pick): 实现方案评分函数（红球80% + 蓝球20%）"
```
...
### Task 5: 重写 print_analysis() 和 main()

**Files:**
- Modify: `examples/ai_pick.rs:163-221`（print_analysis 函数）
- Modify: `examples/ai_pick.rs:12-51`（main 函数）

- [ ] **Step 1: 替换 print_analysis() 函数**
...
```bash
cargo run --example ai_pick 2>&1
```
Expected: 运行成功，输出表格化的分析结果和6组推荐方案

- [ ] **Step 4: Commit**

```bash
git add examples/ai_pick.rs
git commit -m "feat(ai_pick): 表格化输出分析结果和推荐方案"
```
...
**Files:**
- Modify: `examples/ai_pick.rs`（全局）

- [ ] **Step 1: 检查编译警告**

```bash
cargo build --example ai_pick 2>&1 | grep warning
```
Expected: 无警告
...
cargo run --example ai_pick 2>&1
```
Expected: 从缓存读取，无需网络请求，输出与上次一致

- [ ] **Step 4: 最终 Commit**

```bash
git add examples/ai_pick.rs
git commit -m "chore(ai_pick): 清理代码，确认编译无警告测试通过"
```
